...
public class MicPreferences {
  public final static int LEN1 = 12, LEN2 = 17, DELAY = 0, MBASE = 1, CBASE = 2, CSIZE = 3,
                          SBASE = 4, SSIZE = 5, SP = 6, LV = 7, CPP = 8, MIC = 9, MAC = 10,
                          IJVM = 11, TRACK = 12, MHI = 13, SHI = 14, STO = 15, RES = 16, BRK = LEN2;
  ...
  private ButtonGroup[] bg = {new ButtonGroup(), new ButtonGroup(), new ButtonGroup(),
      new ButtonGroup(), new ButtonGroup()};
  private String[] titles = {"Demo Delay Duration (ms)", "Method Area Base", "Constant Pool Base",
      "Constant Pool Size", "Stack Area Base", "Stack Area Size", "Initial SP (Hex)",
      "Initial LV (Hex)", "Initial CPP (Hex)", "Default Microprogram", "Default Macroprogram",
      "Default IJVM Configuration", "Memory Area Tracking", "Method Highlighting",
      "CP/Stack Highlighting", "Microstore Highlighting", "Screen Resolution"};
  ...
  public void show(Properties props) {
    ...
    int n = JOptionPane.showOptionDialog(frame, mainPanel, "Edit Preferences",
  ...
  public void load(Properties props) {
    ...
    try {
      ...
      JOptionPane.showMessageDialog(
          frame, "IO Error", "Error: " + ex.getMessage(), JOptionPane.ERROR_MESSAGE);
    }
  }
  ...
  public void loadDefaults(Properties props) {
    ...
    try {
      ...
      try {
        in = getClass().getResourceAsStream("/" + frame.defaultPropFile);
      } catch (Exception ex1) {
        setProperties(props);
        return;
      }
  ...
  private void loadIJVM() {
    ...
    try {
      try {
        ...
        try {
          in = getClass().getResourceAsStream("/" + frame.defaultIjvmFile);
          MicTools.initIJVM(in);

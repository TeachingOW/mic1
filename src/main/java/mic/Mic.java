...
public class Mic extends JFrame {
  ...
  public final static int ACTIONCOUNT = 13, LOADMAC = 0, LOADMIC = 1, LOADJAS = 2, LOADMAL = 3,
                          PREF = 4, LOADPREF = 5, SAVEPREF = 6, VIEWMAL = 7, IJVM = 8, MIC = 9,
                          HELP = 10, HELPON = 11, ABOUT = 12, FILEMENULEVEL = 4, EDITMENULEVEL = 7,
                          MALMENULEVEL = 8, ASSLOADMENULEVEL = 10;
  ...
  public String defaultPropFile = "mic1.properties", defaultIjvmFile = "ijvm.conf", ijvmFile,
                helpFile = "help.html";
  ...
  private String[] tipList = {"Load IJVM program ...", "Load Micro program ...",
      "Assemble / Load JAS file ...", "Assemble / Load MAL file ...", "Edit Preferences ...",
      "Load Preferences ...", "Save Preferences ...", "View Microstore",
      "Current JAS Assemble / Load", "Current MAL Assemble / Load", title + " Help",
      title + " Help On ...", "About " + title};
  ...
  private JMenu menuFile = new JMenu("File"), menuEdit = new JMenu("Preferences"),
                menuMal = new JMenu("Microcode Store"), menuAssLoad = new JMenu("Assemble/Load"),
  ...
  private String internalMicName = "/mic1ijvm.mic1";
  ...
  public Mic(String micFile) {
    ...
    ctlcvs.setBounds(MID[reslvl], MEMSIZ[reslvl] + CTLSIZ[reslvl] + 15, CtlCanvas.WID[reslvl],
        CtlCanvas.HTH[reslvl]);
    ...
    for (int i = 0; i < ACTIONCOUNT; i++) {
      ...
      } else if (i < EDITMENULEVEL)
        menuEdit.add(actionList[i]);
      else if (i < MALMENULEVEL)
        menuMal.add(actionList[i]);
      else if (i < ASSLOADMENULEVEL)
        menuAssLoad.add(actionList[i]);
      else
  ...
  private void setTitle() {
    StringBuffer sbuf = new StringBuffer(
        title + " (" + micName + ".mic1) - " + macName + ((macName.length() == 0) ? "" : ".ijvm"));
    if (malFile == null && jasFile == null)
      setTitle(sbuf.toString());
    else {
      sbuf.append("       ");
      if (malFile != null)
        sbuf.append("MAL: " + malFile + "   ");
      if (jasFile != null)
        sbuf.append("JAS: " + jasFile);
      setTitle(sbuf.toString());
    }
  }
  ...
  public void mStoHilite(int mpc) {
    if (MicText.getCurrent() == null || !isMStoHL())
      return;
    MicText.getCurrent().hilite(mpc);
  }

  public void mStoUnHilite() {
    if (MicText.getCurrent() == null)
      return;
    ...
    try {
      ...
      try {
        ...
        JOptionPane.showMessageDialog(
            this, "Unable to Load Microcode Program", "Error", JOptionPane.ERROR_MESSAGE);
        return;
      }
    }
    String name = filename;
    if (name.indexOf(".mic1") > 0)
      name = name.substring(0, name.indexOf(".mic1"));
    micName = name;
  }
  ...
  private void getDefaultMacFile(String filename) {
    ...
    try {
      ...
      JOptionPane.showMessageDialog(
          this, "IO Error Reading\n" + filename, "Error", JOptionPane.ERROR_MESSAGE);
      return;
    }
    String name = filename;
    if (name.indexOf(".ijvm") > 0)
      name = name.substring(0, name.indexOf(".ijvm"));
    macName = name;
  }
  ...
  private void loadMic(File currentFile) {
    ...
    if (malFile != null && !tmpFile.equals(malFile))
      malFile = null;
    setTitle();
    repaint();
  }
  ...
  private boolean loadMac(File currentFile) {
    ...
    String name = currentFile.getName();
    if (name.endsWith(".ijvm"))
      name = name.substring(0, name.indexOf(".ijvm"));
    macName = name;
    File tmpFile = new File(currentFile.getParent(), macName + ".jas");
    if (jasFile != null && !tmpFile.equals(jasFile))
      jasFile = null;
    setTitle();
    repaint();
    return true;
  ...
  private void assAndLoad(boolean newFile) {
    ...
    if (jasFile == null || newFile) {
      if ((inFile = getMacMic("Assemble / Load", jasff)) == null)
        return;
      String name = inFile.getName();
      if (!name.endsWith(".jas")) {
        JOptionPane.showMessageDialog(
            this, "Source file must be .jas file", "Error", JOptionPane.ERROR_MESSAGE);
        return;
      }
    } else {
      inFile = jasFile;
    ...
    outFile = new File(
        inFile.getParent(), inFile.getName().substring(0, inFile.getName().length() - 4) + ".ijvm");
  ...
  private void micAndLoad(boolean newFile) {
    ...
    if (malFile == null || newFile) {
      if ((inFile = getMacMic("Assemble / Load", malff)) == null)
        return;
      String name = inFile.getName();
      if (!name.endsWith(".mal")) {
        JOptionPane.showMessageDialog(
            this, "Source file must be .mal file", "Error", JOptionPane.ERROR_MESSAGE);
        return;
      }
    } else {
      inFile = malFile;
    ...
    outFile = new File(
        inFile.getParent(), inFile.getName().substring(0, inFile.getName().length() - 4) + ".mic1");
